...
public class StringSet {
    ...
    public static Set<String> findUniqueWords(){
        ...
        String[] splitedText = text.toLowerCase().replaceAll("[^a-zA-Z\\s]", "").split("\\s+");
//        Set <String> sortedText = new TreeSet<>(Arrays.asList(splitedText));
        Set <String> sortedText = new TreeSet<>(Arrays.asList(splitedText));
        for (String txt : splitedText ) {
            sortedText.add(txt.trim());
        }
        for (String txt : sortedText){
